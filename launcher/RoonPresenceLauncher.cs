...
namespace RoonPresenceLauncher
{
    internal static class Program
    {
        ...
        private static int Main(string[] args)
        {
            ...
            if (!CommandExists("node", "--version", appDir, "Node.js was not found. Install Node.js LTS, then run this launcher again."))
            {
                Pause();
                return 1;
            }

            if (!CommandExists("npm.cmd", "--version", appDir, "npm was not found. Install Node.js LTS, then run this launcher again."))
            {
                Pause();
                return 1;
            }
            ...
            if (!Directory.Exists(Path.Combine(appDir, "node_modules")))
            {
                ...
                var installCode = Run("npm.cmd", "install", appDir);
                if (installCode != 0)
                {
            ...
            if (!File.Exists(Path.Combine(appDir, ".env")))
            {
                ...
                var setupCode = Run("npm.cmd", "run setup", appDir);
                if (setupCode != 0)
                {
            ...
            return Run("npm.cmd", "start", appDir);
        }
